-- Fix ambiguous column reference error in complete_tile_and_check_stage
-- The error was: column reference "tiles_data" is ambiguous
-- Cause: RETURNS TABLE column name conflicts with upscale_jobs.tiles_data
...
CREATE OR REPLACE FUNCTION complete_tile_and_check_stage(
  ...
  job_tiles_data JSONB,  -- Renamed to avoid conflict with upscale_jobs.tiles_data
  total_stages INT,
  current_stage INT,
...
BEGIN
  ...
  -- Update the tile's status and URL using explicit table alias
  ...
  IF v_current_stage != p_stage THEN
    ...
    RETURN QUERY SELECT 
      ...
      updated_tiles AS job_tiles_data,
