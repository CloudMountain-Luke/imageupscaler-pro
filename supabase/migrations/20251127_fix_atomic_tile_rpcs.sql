-- Fix RPC functions to match actual upscale_jobs schema
-- The previous migration referenced columns that don't exist (updated_at, grid_cols, grid_rows)

-- Drop the functions first (required when changing return type)
...
BEGIN
  ...
  -- Update the job with the new tiles_data (no updated_at column exists)
  UPDATE upscale_jobs
...
-- Fixed: removed grid_cols and grid_rows which don't exist
...
BEGIN
  ...
  -- Update without updated_at (column doesn't exist)
  UPDATE upscale_jobs
  SET tiles_data = updated_tiles
  WHERE id = p_job_id;
END;
